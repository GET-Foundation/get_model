...
checkpoint = torch.load("/pmglocal/alb2281/get/get_ckpts/gbm_Tumor_C3L-03405_lora.ckpt")
...
add_lora_by_name(model, ['region_embed', 'encoder'], lora_config)
...
with torch.autocast(dtype=torch.bfloat16, device_type='cuda'):
    ...
    for i, data in tqdm(enumerate(dl)):
        ...
        for i in range(len(data['gene_name'])):
            ...
            jacobian_norm = np.linalg.norm(jacobians['exp'][str(data['strand'].item())]['region_embed'][i], axis=1)
            ...
            results_dir = "/pmglocal/alb2281/repos/get_model/analysis/results/unnorm_input_finetune_TERT_get"
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)

            peaks.to_csv(f"{results_dir}/unnorm_input_finetune_{data['gene_name'][i]}_get.csv")

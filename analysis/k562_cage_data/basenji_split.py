...
# %%
enformer_seqs_df = pd.read_csv(enformer_seqs, sep="\t", names=["Chromosome", "Start", "End", "group"]).query('group!="train"')
# %%
leaveout_df
...
overlap_bed = pr(leaveout_df.reset_index()).join(pr(enformer_seqs_df), suffix="_enformer").df[leaveout_df.columns.tolist()+['index']].drop_duplicates()

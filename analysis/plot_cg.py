...
sns.clustermap(subset_df, cmap="viridis", method="ward", yticklabels=True, xticklabels=True)
...
merged_df = pd.merge(subset_df, fetal_log_tpm, left_index=True, right_index=True, how="inner")
# %%
# get list of most variable rows
most_variable = merged_df.var(axis=1).sort_values(ascending=False).index[:100]
# %%
subset_df = merged_df.loc[most_variable]
# %%

sns.clustermap(subset_df, cmap="viridis", method="ward", yticklabels=True, xticklabels=True)
# %%

# only include columns with Tumor or Cerebrum in the name
tumor_cerebrum_df = merged_df[[col for col in merged_df.columns if "Tumor" in col or "Cerebrum" in col]]
# %%

most_variable_tumor = tumor_cerebrum_df.var(axis=1).sort_values(ascending=False).index[:50]
tumor_subset_df = tumor_cerebrum_df.loc[most_variable_tumor]
sns.clustermap(tumor_subset_df, cmap="viridis", yticklabels=True, xticklabels=True)
...
# %%
# get list of most variable rows
most_variable = merged_df.var(axis=1).sort_values(ascending=False).index[:50]
# %%
subset_df = merged_df.loc[most_variable]
# %%

...
DATE=`date +%Y%m%d`
OUTPUT_DIR="/pmglocal/xf2217/Expression_Finetune_THP1.mll.Chr4&14.conv20.chrombpnet_bias.revcomp.nolog.nomotifbias.softplus.${DATE}"
...
OMP_NUM_THREADS=1 torchrun --nproc_per_node=4 --rdzv-endpoint=localhost:$PORT get_model/finetune.py \
    ...
    --output_dim 2 \
    --num_motif 637 \
    --model get_finetune_motif_chrombpnet_bias \
    --batch_size 128 \
    --num_workers 32 \
    ...
    --random_shift_peak 0 \
    --pin_mem \
    --peak_name "peaks_negative_tissue_open_exp" \
    --save_ckpt_freq 1 \
    --n_packs 1 \
    --lr 1e-4 \
    --opt adamw \
